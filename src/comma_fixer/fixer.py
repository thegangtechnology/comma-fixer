...
class Fixer:
    ...
    def __check_valid(self, new_entry: str) -> Optional[ParsedEntry]:
        ...
        if paths is not None and len(paths) > 1:
            ...
            for path in paths:
                return self.__construct_processed_entry_from_path(path, tokens, num_cols, num_tokens)
        return None
    
    def __construct_processed_entry_from_path(self, path: list[tuple[int, int]], tokens: list[str], num_cols: int, num_tokens: int) -> list[str]:
    ...
    def __construct_validity_matrix(self, new_entry: str) -> np.array:
        ...
        for token_index, token in enumerate(tokens):
            ...
            for column_index, column_name in enumerate(self.schema.get_column_names()):
                preceding_zero = self.__check_preceding_zero_in_path(validity_matrix, token_index, column_index)
                if preceding_zero == True or token_index == 0:
                    validity_matrix[token_index][column_index] = 0 if self.schema.is_token_valid(token.strip(), column_name) else 1
                    if token_index > 0 and self.schema.has_commas[column_name] == False and validity_matrix[token_index - 1][column_index] == 0:
                        validity_matrix[token_index][column_index] = 1
                    elif len(token) == 0 and self.schema.has_commas[column_name] == True:
                        validity_matrix[token_index][column_index] = 0
                else:
                    continue
                if validity_matrix[token_index][column_index] == 0 and first_valid_index == -1:
                    first_valid_index = column_index
                if token_index != 0 and column_index > furthest_col:
                    break
            furthest_col = first_valid_index if first_valid_index >= furthest_col + 1 else furthest_col
        return validity_matrix
              
    def __check_preceding_zero_in_path(self, validity_matrix: np.array, token_index: int, column_index: int) -> bool:
        if column_index > 0 and token_index > 0:
            if validity_matrix[token_index - 1][column_index - 1] == 0 or validity_matrix[token_index - 1][column_index] == 0:
                return True
            ...
            for column in range(num_columns + 1):
                if row < num_tokens and column < num_columns:
                    if validity_matrix[row][column] != 1:
                        G.add_edge((row, column), (row+1, column+1), weight = validity_matrix[row][column])
                        G.add_edge((row, column), (row+1, column), weight = validity_matrix[row][column])
